# ***********************************************************************
# 이 프로그램은 사용자별 GSR과 SCR 데이터를 이벤트 구간별로 그래프를 그려준다
#                                                         @Huhn Kim, 2024
# ***********************************************************************
...
# 폴더 경로 설정
gsr_folder = os.path.expanduser("processed_imf3_GSRoutput/")
user_folder = os.path.expanduser("User_event_1/")
output_folder = "GSR_SCR_graphs_P14"  # 출력 폴더 경로 설정
...
for user_file in user_files:
    ...
    print(f"Filtered GSR data points: {len(event_data)}")

    # 데이터가 비어있는지 확인
    if event_data.empty:
        print(f"No matching GSR data found for the given time range")
        print(f"Available GSR data dates: {gsr_data['date'].unique()}")
        print(f"GSR data time range: {gsr_data['time'].min()} ~ {gsr_data['time'].max()}")
        continue
    ...
    plt.title(f'{user_file}: GSR & SCR Values')
    ...
    # 첫 번째 서브플롯 - GSR 값
    plt.subplot(2, 1, 1)
    plt.plot(event_data['time_seconds'], event_data['GSR'], linestyle='-', color='g')  # 마커 없이 선만 그리기
    plt.ylabel('GSR', labelpad=15)

    # y축 범위 자동 조정 또는 수동으로 설정
    plt.ylim(min(event_data['GSR']), max(event_data['GSR']))  # 빈 공간 최소화
    ...
    ax1.plot(event_data['time_seconds'], event_data['SCR_z'], linestyle='-', color='b')  # 마커 없이 선만 그리기
    ax1.set_ylabel('SCR z-score', labelpad=3)

    # y축 범위 조정
    plt.ylim(min(event_data['SCR_z']), max(event_data['SCR_z']))  # 빈 공간 최소화
    plt.xlim(x_min, x_max)  # x축 범위를 동일하게 설정
    ...
    for i in range(0, len(user_df) - 1, 2):
        ...
        # y축 최소값과 최대값
        y_min = min(event_data['SCR_z'])
        y_max = max(event_data['SCR_z'])
    ...
    #plt.savefig(f'GSR_SCR_graphs/P{user_number}_{support_status}_gsr.png') # <<---- 이미지 파일로 한번에 저장할 때

    # 파일 이름 생성
    output_filename = f'P{user_number}_{support_status}_gsr.png'
