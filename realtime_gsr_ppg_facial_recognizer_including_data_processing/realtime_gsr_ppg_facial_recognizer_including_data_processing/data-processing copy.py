...
# 폴더 경로 설정
gsr_folder = r"C:\Users\aloho\Github\realtime_gsr_ppg_facial_recognizer_including_data_processing\Processed_GSR"  # 처리된 GSR CSV 폴더
event_folder = os.path.expanduser("timeline/")  # 사용자별 이벤트의 시작 및 끝 시간 데이터
...
for i in range(0, len(output_df), rows_per_file):
    ...
    output_df.iloc[i:i + rows_per_file].to_csv(out_path, index=False, encoding="utf-8-sig")
    file_count += 1
