# ***********************************************************************
# 이 프로그램은 사용자별 Driving 데이터를 이벤트 구간별로 그래프를 그려준다
#                                                       @Huhn Kim, 2024
# ***********************************************************************
...
# 폴더 경로 설정
gsr_folder = os.path.expanduser("User_driving/")
...
for user_file in user_files:
    ...
    plt.title(f'{user_file}: Speed, Brake, Handle Operation')
    ...
    # 첫 번째 서브플롯 - GSR 값
    plt.subplot(3, 1, 1)
    plt.plot(event_data['time_seconds'], event_data['speed'], linestyle='-', color='black')  # 마커 없이 선만 그리기
    plt.ylabel('speed', labelpad=15)

    # y축 범위 자동 조정 또는 수동으로 설정
    plt.ylim(min(event_data['speed']), max(event_data['speed']))  # 빈 공간 최소화
    ...
    # 첫 번째 서브플롯 - GSR 값
    plt.subplot(3, 1, 2)
    plt.plot(event_data['time_seconds'], event_data['brake'], linestyle='-', color='g')  # 마커 없이 선만 그리기
    plt.ylabel('brake', labelpad=15)

    # y축 범위 자동 조정 또는 수동으로 설정
    plt.ylim(min(event_data['brake']), max(event_data['brake']))  # 빈 공간 최소화
    plt.xlim(x_min, x_max)  # x축 범위를 동일하게 설정
    ...
    ax1.plot(event_data['time_seconds'], event_data['handle'], linestyle='-', color='b')  # 마커 없이 선만 그리기
    ax1.set_ylabel('handle', labelpad=15)

    # y축 범위 조정
    plt.ylim(min(event_data['handle']), max(event_data['handle']))  # 빈 공간 최소화
    plt.xlim(x_min, x_max)  # x축 범위를 동일하게 설정
    ...
    for i in range(0, len(user_df) - 1, 2):
        ...
        # y축 최소값과 최대값
        y_min = min(event_data['handle'])
        y_max = max(event_data['handle'])
        ...
        plt.text(x=event_start_time_seconds, y=y_min - 0.1 * (y_max - y_min + 180), s=str(event_name[event_label-1]), color='black',
    ...
    plt.savefig(f'Driving_graphs/P{user_number}_{support_status}_driving.png') #<<---- 이미지 파일로 한번에 저장할 때
