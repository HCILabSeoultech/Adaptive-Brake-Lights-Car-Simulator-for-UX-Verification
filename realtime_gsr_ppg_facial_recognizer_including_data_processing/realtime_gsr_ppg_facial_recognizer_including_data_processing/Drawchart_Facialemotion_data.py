# ***********************************************************************
# 이 프로그램은 사용자별 표정인식 감정비율 데이터를 이벤트 구간별로 그래프를 그려준다
#                                                           @Huhn Kim, 2024
# ***********************************************************************
...
for user_file in user_files:
    ...
    plt.title(f'{user_file}: Facial emotion graph')
    ...
    # 첫 번째 서브플롯 - Neutral 값
    plt.subplot(7, 1, 1)
    plt.plot(event_data['time_seconds'], event_data['Neutral'], linestyle='-', color='black')  # 마커 없이 선만 그리기
    plt.ylabel('Neutral', labelpad=15)

    # y축 범위 자동 조정 또는 수동으로 설정
    plt.ylim(min(event_data['Neutral']), max(event_data['Neutral']))  # 빈 공간 최소화
    ...
    # 첫 번째 서브플롯 - GSR 값
    plt.subplot(7, 1, 2)
    plt.plot(event_data['time_seconds'], event_data['Sad'], linestyle='-', color='gray')  # 마커 없이 선만 그리기
    plt.ylabel('Sad', labelpad=15)

    # y축 범위 자동 조정 또는 수동으로 설정
    plt.ylim(min(event_data['Sad']), max(event_data['Sad']))  # 빈 공간 최소화
    plt.xlim(x_min, x_max)  # x축 범위를 동일하게 설정
    ...
    ax1.plot(event_data['time_seconds'], event_data['Angry'], linestyle='-', color='pink')  # 마커 없이 선만 그리기
    ax1.set_ylabel('Angry', labelpad=15)

    # y축 범위 조정
    plt.ylim(min(event_data['Angry']), max(event_data['Angry']))  # 빈 공간 최소화
    plt.xlim(x_min, x_max)  # x축 범위를 동일하게 설정
    ...
    # 첫 번째 서브플롯 - GSR 값
    plt.subplot(7, 1, 4)
    plt.plot(event_data['time_seconds'], event_data['Disgust'], linestyle='-', color='b')  # 마커 없이 선만 그리기
    plt.ylabel('Disgust', labelpad=15)

    # y축 범위 자동 조정 또는 수동으로 설정
    plt.ylim(min(event_data['Disgust']), max(event_data['Disgust']))  # 빈 공간 최소화
    plt.xlim(x_min, x_max)  # x축 범위를 동일하게 설정
    ...
    # 첫 번째 서브플롯 - GSR 값
    plt.subplot(7, 1, 5)
    plt.plot(event_data['time_seconds'], event_data['Fear'], linestyle='-', color='g')  # 마커 없이 선만 그리기
    plt.ylabel('Fear', labelpad=15)

    # y축 범위 자동 조정 또는 수동으로 설정
    plt.ylim(min(event_data['Fear']), max(event_data['Fear']))  # 빈 공간 최소화
    plt.xlim(x_min, x_max)  # x축 범위를 동일하게 설정
    ...
    # 첫 번째 서브플롯 - GSR 값
    plt.subplot(7, 1, 6)
    plt.plot(event_data['time_seconds'], event_data['Happy'], linestyle='-', color='g')  # 마커 없이 선만 그리기
    plt.ylabel('Happy', labelpad=15)

    # y축 범위 자동 조정 또는 수동으로 설정
    plt.ylim(min(event_data['Happy']), max(event_data['Happy']))  # 빈 공간 최소화
    plt.xlim(x_min, x_max)  # x축 범위를 동일하게 설정
    ...
    # 첫 번째 서브플롯 - GSR 값
    plt.subplot(7, 1, 7)
    plt.plot(event_data['time_seconds'], event_data['Surprise'], linestyle='-', color='g')  # 마커 없이 선만 그리기
    plt.ylabel('Surprise', labelpad=15)

    # y축 범위 자동 조정 또는 수동으로 설정
    plt.ylim(min(event_data['Surprise']), max(event_data['Surprise']))  # 빈 공간 최소화
    plt.xlim(x_min, x_max)  # x축 범위를 동일하게 설정
    ...
    for i in range(0, len(user_df) - 1, 2):
        ...
        # y축 최소값과 최대값
        y_min = min(event_data['Surprise'])
        y_max = max(event_data['Surprise'])
        ...
        plt.text(x=event_start_time_seconds, y=y_min - 0.1 * (y_max - y_min + 0.04), s=str(event_name[event_label-1]), color='black',
    ...
    plt.savefig(f'Emotion_graphs/P{user_number}_{support_status}_emotion.png') #<<---- 이미지 파일로 한번에 저장할 때
