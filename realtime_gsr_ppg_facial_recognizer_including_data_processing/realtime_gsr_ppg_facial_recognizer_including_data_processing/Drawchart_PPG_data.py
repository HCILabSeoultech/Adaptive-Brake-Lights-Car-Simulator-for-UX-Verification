# ***********************************************************************
# 이 프로그램은 사용자별 PPG 데이터를 이벤트 구간별로 그래프를 그려준다
#                                                      @Huhn Kim, 2024
# ***********************************************************************
...
for user_file in user_files:
    ...
    plt.title(f'{user_file}: PPG Values')
    ...
    # 첫 번째 서브플롯 - GSR 값
    plt.plot(event_data['time_seconds'], event_data['PPG'], linestyle='-', color='g')  # 마커 없이 선만 그리기
    plt.ylabel('PPG', labelpad=15)

    # y축 범위 자동 조정 또는 수동으로 설정
    plt.ylim(min(event_data['PPG']), max(event_data['PPG']))  # 빈 공간 최소화
    ...
    for i in range(0, len(user_df) - 1, 2):
        ...
        # y축 최소값과 최대값
        y_min = min(event_data['PPG'])
        y_max = max(event_data['PPG'])
        ...
        plt.text(x=event_start_time_seconds, y=y_min - 0.1 * (y_max - y_min + 200), s=str(event_name[event_label-1]), color='black',
    ...
    plt.savefig(f'PPG_graphs/P{user_number}_{support_status}_ppg.png')
