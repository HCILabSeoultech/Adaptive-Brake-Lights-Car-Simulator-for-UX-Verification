...
for event_file in event_files:
    ...
    for i in range(0, len(user_df) - 1, 2):
        try:
            ...
            # ---------- GSR ----------
            def has_valid(series_name):
                return (series_name in event_data and
                        event_data[series_name].notna().sum() > 0)
            ...
            if has_valid('GSR'):
                ...
                gsr_peaks = len(find_peaks(gsr_series.dropna().tolist())[0])
            else:
                gsr_avg = gsr_std = gsr_max = gsr_min = gsr_range = gsr_peaks = None

            gsr_z_avg = event_data['GSR_z'].mean() if has_valid('GSR_z') else None
            scr_z_avg = event_data['SCR_z'].mean() if has_valid('SCR_z') else None
            ...
            if has_valid('SCR'):
                ...
                scr_peaks = len(find_peaks(scr_series.dropna().tolist())[0])
            else:
                scr_avg = scr_std = scr_max = scr_min = scr_range = scr_peaks = None

            # 안정화 시간
            if has_valid('GSR') and 'time(ms)' in event_data and event_data['time(ms)'].notna().sum() > 0:
                result = time_to_stable(event_data, 'GSR')
                gsr_time_to_stable = result[2] if result else None
            else:
                gsr_time_to_stable = None

            scr_duration_unstable = total_time_below_threshold(event_data, 2) if has_valid('SCR_z') else None
            ...
            if has_valid('PPG'):
                ...
                ppg_peaks = len(find_peaks(ppg_series.dropna().tolist())[0])
                try:
            ...
            if filtered_ppg.size:
                try:
                    ...
                    HR = m.get('bpm'); HRV = m.get('ibi'); SDNN = m.get('sdnn')
                    RMSSD = m.get('rmssd'); pNN50 = m.get('pnn50')
                except Exception:
                    pass

            # ---------- EYE ----------
            eye_cols = ['Leye_X','Leye_Y','Reye_X','Reye_Y','Leye_open','Reye_open']
            has_eye = all(c in event_data.columns for c in eye_cols) and event_data[eye_cols].notna().any(axis=None)
            if has_eye and len(event_data) >= 2:
                ...
                for j in range(1, len(event_data)):
                    prev = event_data.iloc[j-1]; cur = event_data.iloc[j]
                    if not any(pd.isna(prev[c]) or pd.isna(cur[c]) for c in eye_cols):
                        ...
                        if prev['Leye_open'] == 1 and cur['Leye_open'] == 0: leye_blink += 1
                        if prev['Reye_open'] == 1 and cur['Reye_open'] == 0: reye_blink += 1
            ...
            emotion_present = [e for e in emotion_labels if has_valid(e)]
            if emotion_present:
                emotion_data = event_data[emotion_present]
                primary = emotion_data.apply(lambda r: r.idxmax() if r.notna().any() else None, axis=1)
                secondary = emotion_data.apply(lambda r: r.dropna().nlargest(2).index[1] if r.dropna().size >= 2 else None, axis=1)
                from collections import Counter
                mc = Counter(primary.dropna()); sc = Counter(secondary.dropna())
